...
export async function generateStaticParams() {
    const items = getAllContent('tutorials');
...
export async function generateMetadata({ params }: TutorialPageProps) {
    ...
    return {
        title: `${item.meta.title} - Soplang Tutorials`,
...
export default function TutorialPage({ params }: TutorialPageProps) {
    const slug = params.slug.join('/');
    const item = getContentBySlug('tutorials', slug);
    const allItems = getAllContent('tutorials');
    ...
    if (itemIndex > 0) {
        ...
        prevPage = {
            ...
            title: prev.title,
            description: prev.description || 'Previous tutorial',
        };
    }
    ...
    if (itemIndex > -1 && itemIndex < allItems.length - 1) {
        ...
        nextPage = {
            ...
            title: next.title,
            description: next.description || 'Next tutorial',
        };
    }
